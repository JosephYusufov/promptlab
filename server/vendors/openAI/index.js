...
export const generatePrompt = async (prompt, airesponse, useresponse, iterations=0, feedbackVar=null) => {
    ...
    const feedbackPrompt = 'I gave this prompt to chatgpt-3.5: ' + prompt + '. After I fed this prompt to the' +
        ' model, this is what I got: ' + airesponse + ' This marks the end of the model output. However, I was hoping' +
        ' for an output more like the text below: ' + useresponse + ' This marks the end of the ideal output. How can I ' +
    ...
    const feedback =  await applyFeedback(prompt, response.data.choices[0].message.content)
    ...
    if (newPromptText.includes('No prompt found')) {
        ...
        return generatePrompt(prompt, airesponse, useresponse, iterations=++iterations, feedbackVar=feedbackText)
    } else {
...
export const applyFeedback = async (originalPrompt, feedback) => {
    ...
    const getFeedback = 'Here is an original prompt for this LLM model: ' + originalPrompt + ' This marks' +
        ' the end of the original prompt. Here is the feedback I received for this prompt: ' + feedback + ' This ' +
...
export const checkForPrompt = async (potPrompt) => {
    ...
    const modelText = 'A valid prompt is defined as any request that could be interpreted ' +
